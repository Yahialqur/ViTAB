# """
# Model handler for Gemma models
# """
# import os
# import getpass

# # Set Hugging Face cache directory BEFORE any huggingface imports
# username = getpass.getuser()
# scratch_cache_dir = f'/scratch/{username}/.cache/huggingface'

# # Set all HF cache environment variables
# os.environ['HF_HOME'] = scratch_cache_dir
# os.environ['TRANSFORMERS_CACHE'] = f'{scratch_cache_dir}/transformers'
# os.environ['HF_HUB_CACHE'] = f'{scratch_cache_dir}/hub'
# os.environ['HF_DATASETS_CACHE'] = f'{scratch_cache_dir}/datasets'  

# # Create cache directory if it doesn't exist
# os.makedirs(scratch_cache_dir, exist_ok=True)
# os.makedirs(f'{scratch_cache_dir}/transformers', exist_ok=True)
# os.makedirs(f'{scratch_cache_dir}/hub', exist_ok=True)
# os.makedirs(f'{scratch_cache_dir}/datasets', exist_ok=True)

# print(f"Hugging Face cache directory set to: {scratch_cache_dir}")
...
#     logits: Optional[torch.Tensor] = None  # First token logits for confidence extraction
#     token_probabilities: Optional[Dict[int, float]] = None  # Token ID -> probability
...
#         # Prepare messages with image + text
...
#             return self.generate_with_image(prompt, image, return_logits)
#         else:
...
#         self.config = config
#         self.current_model: Optional[InternVLModel] = None
#         self.current_model_name: Optional[str] = None

#     def get_model(self, model_name: str) -> InternVLModel:
#         """
...
class InternVLModel:
    """Handler for InternVL models, following official HF documentation"""
    ...
    def generate_text_only(
        ...
        InternVL 3.5 accepts text-only chat-style inputs by providing a single
        text content block to the processor's chat template.
        """
        if not self._loaded:
            self.load()
    ...
    def generate_with_image(
        ...
        # Prepare messages with image + text following InternVL official example
        # For few-shot with example image, include it before the main prompt
        content = []
        if example_image is not None:
            ...
            # Single image case (zero-shot or CoT) - follow official InternVL example structure
            if "<IMAGE>" in prompt:
                raise ValueError(
        ...
        # Apply chat template and tokenize with image (following official InternVL example)
        inputs = self.processor.apply_chat_template(
            messages,
            add_generation_prompt=True,
            tokenize=True,
    ...
    def generate(
        ...
        """
        Unified generate method for InternVL inputs.

        Args:
            prompt: The text prompt
            image: Optional PIL Image (required for image representations)
            return_logits: If True, return token logits for confidence extraction
